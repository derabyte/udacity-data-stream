...
# TO-DO: create a StructType for the Kafka stedi-events topic which has the Customer Risk JSON that
# comes from Redis- before Spark 3.0.0, schema inference is not automatic
kafkaStediEventsSchema = StructType([
...
spark = SparkSession.builder.appName("kafka-redis").getOrCreate()
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you
# started the spark stream
redisStream = spark.readStream \
    .format('kafka') \
...
redisStreamValue = redisStream.selectExpr("cast(value as string) value")
...
redisStreamValue.withColumn("value", from_json("value", redisSchema))\
    .select(col('value.existType'),
            col('value.ch'),
            col('value.incr'),
            col('value.zSetEntries'))\
    .createOrReplaceTempView('RedisSortedSet')

# TO-DO: execute a sql statement against a temporary view, which statement takes the element field from the 0th element
# in the array of structs and create a column called encodedCustomer the reason we do it this way is that the syntax
# available select against a view is different than a dataframe, and it makes it easy to select the nth element of an
# array in a sql column
...
decodedCustomerDF = decodedCustomerDF.withColumn("customer", from_json("customer", customerJSONSchema)) \
    .select(col('customer.*')) \
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you
# started the spark stream
kafkaRawStediStream = spark.readStream \
    .format('kafka') \
    .option('kafka.bootstrap.servers', 'kafka:19092') \
...
# TO-DO: cast the value column in the streaming dataframe as a STRING
stediEventValueDF = kafkaRawStediStream.selectExpr('cast(value as string) value')
...
stediEventValueDF = stediEventValueDF.withColumn("value", from_json("value", kafkaStediEventsSchema)) \
    .select("value.*") \
